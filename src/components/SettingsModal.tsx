...
interface SettingsModalProps {
  ...
  requesters: string[];
  sprints: Sprint[];
  onUpdateDesigners: (designers: Designer[]) => void;
  onUpdateRequesters: (requesters: string[]) => void;
  onUpdateSprints: (sprints: Sprint[]) => void;
}

export const SettingsModal: React.FC<SettingsModalProps> = ({
  isOpen, onClose, designers, requesters, sprints, onUpdateDesigners, onUpdateRequesters, onUpdateSprints
}) => {
  ...
  const addDesigner = () => {
    ...
    onUpdateDesigners([...designers, newDesigner]);
    setNewDesignerName('');
  };
  ...
  const addRequester = () => {
    ...
    if (!requesters.includes(newRequesterName)) {
        onUpdateRequesters([...requesters, newRequesterName]);
    }
    setNewRequesterName('');
  };

  const removeRequester = (name: string) => {
    onUpdateRequesters(requesters.filter(r => r !== name));
  };
  ...
  const addSprint = () => {
    ...
    onUpdateSprints([...sprints, sprint]);
    setNewSprint({ name: '', startDate: '', endDate: '' });
  };

  const removeSprint = (id: string) => {
    onUpdateSprints(sprints.filter(s => s.id !== id));
  };

  const toggleSprintActive = (id: string) => {
    onUpdateSprints(sprints.map(s => ({
        ...s,
        isActive: s.id === id ? true : false // Only one active sprint
    })));
  };
