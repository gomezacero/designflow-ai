      ...
      {viewMode === 'board' ? (
        <div className="flex gap-4 overflow-x-auto pb-4 w-full snap-x snap-mandatory md:snap-none min-h-[300px]">
            {Object.values(Status).map((status) => {
                // Get tasks for column and sort automatically by priority
                const columnTasks = visibleTasks
                    .filter(t => t.status === status)
                    .sort((a, b) => {
                        // Apply automatic sorting for active columns (Todo, In Progress, Review)
                        // Done status usually stays sorted by completion date (preserved from visibleTasks)
                ...
                return (
                    ...
                    >
                        ...
                        {/* Drop Zone */}
                        <div className="flex-1 rounded-2xl bg-gray-100/50 border border-gray-200/50 p-2.5 space-y-3 overflow-y-auto custom-scrollbar">
                            {columnTasks.length > 0 ? (
                                columnTasks.map((task) => (
                                    <div
                                        ...
                                        className="transition-transform active:scale-95 active:rotate-1"
                                    >
                                        <TaskCard
                                            task={task}
