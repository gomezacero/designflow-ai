...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ task, onClose, onUpdate, onDelete, designers, requesters }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[24px] shadow-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden flex flex-col animate-[floatUp_0.2s_ease-out]">

                {/* Header */}
                <div className="flex items-start justify-between px-8 py-6 border-b border-gray-100">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">{task.sprint}</span>
                            <span className="text-gray-300">•</span>
                            <span className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${task.priority === Priority.HIGH || task.priority === Priority.CRITICAL ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'}`}>
                                {task.priority}
                            </span>
                        </div>
                        <h2 className="text-2xl font-bold text-gray-900 leading-tight">{task.title}</h2>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600" aria-label="Close task details">
                        <X size={24} />
                    </button>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        <div className="md:col-span-8 space-y-8">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-3">
                                <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Brief / Description</h3>
                        ...
                        <div className="md:col-span-4 space-y-6">
                            ...
                            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                                <label className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Current Status</label>
                                <div className="relative">
                                    <select
                                    ...
                                    ${task.status === Status.DONE ? 'border-green-100 bg-green-50 text-green-700' :
                                                task.status === Status.IN_PROGRESS ? 'border-blue-100 bg-blue-50 text-blue-700' :
                                                    task.status === Status.REVIEW ? 'border-purple-100 bg-purple-50 text-purple-700' :
                            ...
                            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-5">
                                ...
                                <div>
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Assigned Designer</span>
                                    <div className="relative">
                                        <select
                                            className="w-full appearance-none pl-12 pr-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm font-medium outline-none focus:border-blue-500 transition-colors"
                                ...
                                <div>
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Requester</span>
                                    <div className="relative">
                                        <select
                                            className="w-full appearance-none pl-8 pr-4 py-2 rounded-xl border border-gray-200 bg-gray-50 text-sm font-medium outline-none focus:border-blue-500 transition-colors"
                            ...
                            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
                                <div>
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Due Date</span>
                                ...
                                <div>
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-2">Estimation</span>
                                    <div className="flex items-center gap-2 text-sm text-gray-700 bg-gray-50 px-3 py-2 rounded-lg border border-gray-100">
                            ...
                            {onDelete && (
                                <button
                                    ...
                                    className="w-full py-3 text-red-500 font-bold text-sm bg-red-50 hover:bg-red-100 rounded-xl transition-colors border border-red-100"
                                >
                                    Delete Task
                                </button>
                            )}
